...
function DashboardAluno({ user }: { user: User }) {
  ...
  }, [user?.uid, user?.perfis, user?.url_foto]) // Dependências específicas

  useEffect(() => {
    // Aguardar carregamento da autenticação
    if (!user?.uid) return
    
    console.log("🚀 Dashboard: Usuário carregado, iniciando carregamento de dados")
    carregarDadosDashboard(user.uid)
  }, [user?.uid]) // Apenas user?.uid como dependência

  const carregarDadosDashboard = async (userId: string) => {
    setLoading(true)
    try {
  ...
  const handleRefresh = async () => {
    if (!user?.uid || refreshing || loading) return
