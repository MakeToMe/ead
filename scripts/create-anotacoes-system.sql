...
-- Usando nossa estrutura de autenticação própria

-- Tabela para armazenar anotações dos usuários
CREATE TABLE IF NOT EXISTS rarcursos.anotacoes_usuario (
  id UUID DEFAULT gen_random_uuid() PRIMARY KEY,
  
  -- Referências para nossas tabelas existentes
  usuario_uid VARCHAR(255) NOT NULL REFERENCES rarcursos.users(uid) ON DELETE CASCADE,
